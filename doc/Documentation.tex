...
\section{Conclusion}
\noindent The StreamGraph project aimed to simplify the usage of StreamIt. Specificly to alleviate the 
need for deep understanding of the syntax for topological constructs. Since StreamGraph is partly a 
graphical programming language it inherits some of the advantages of a graphical programming language. 
Namely having a better overwiev over the over all structure of most programms. While this is no longer 
the case for large programs the alternative (the large program in textual form) has the same problem.\\
Additionaly since StreamGraph uses StreamIt code in the description of the filters it also gains the 
benefit of having a clear, structured and detailed descriptions of the inner workings of each filter.
Especially the detailedness is sometimes a problem for pure graphical programming languages.\\
\\
Through the abstraction of topological constructs in the graph StreamGraph succeds in simplifying the 
connecting of the filters and through that nearly removes the need to know the syntax of topolo0gical
constructs in StreamIt.\\
The abstraction of split-join constructs into the nodes and with that the creation of a directional 
graph instead of a pipeline based topology simplifies the connecting of nodes but gives at the same 
time rise to the necessety to enforce the rules of a pipeline based topology in the graph or to make 
sure that every graph can be converted into such a pipeline based structure. Both of these posibilities 
are not easy to accomplish. StreamGraph tries and struggles to convert specific cases in the graph to a 
pipeline based structure and to prohibit the creation of every other case. As mentioned in 
\ref{feature:automatic_compatability} and \ref{bug:incorrect_graphs} StreamGraph still has some problems 
with both cases.\\
All in all it is possible to depict a pipeline based structure through a graph in a meaningfull manner.
StreamGraph is in that regard to be viewed as a proof of concept. With further work it is possible to 
expand on the existing StreamGraph project to bring it to its full potential.
